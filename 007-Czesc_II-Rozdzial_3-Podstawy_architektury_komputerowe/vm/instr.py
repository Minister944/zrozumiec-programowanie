...
def VST(vm, args):
    if not vm.mem.store_dword(vm.reg(args[0]).v, vm.reg(args[1]).v):
        vm.interrupt(vm.INT_MEMORY_ERROR)
...
def VSTB(vm, args):
    if not vm.mem.store_byte(vm.reg(args[0]).v, vm.reg(args[1]).v & 0xFF):
        vm.interrupt(vm.INT_MEMORY_ERROR)


def VADD(vm, args):
    vm.reg(args[0]).v = (vm.reg(args[0]).v + vm.reg(args[1]).v) & 0xFFFFFFFF
...
def VDIV(vm, args):
    if vm.reg(args[1]).v == 0:
        ...
        vm.reg(args[0]).v = vm.reg(args[0]).v // vm.reg(args[1]).v
...
def VSHL(vm, args):
    vm.reg(args[0]).v = (vm.reg(args[0]).v << (vm.reg(args[1]).v & 0x1F)) & 0xFFFFFFFF


def VSHR(vm, args):
    vm.reg(args[0]).v = vm.reg(args[0]).v >> (vm.reg(args[1]).v & 0x1F)
